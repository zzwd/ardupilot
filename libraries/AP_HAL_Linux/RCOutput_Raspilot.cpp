...
void RCOutput_Raspilot::init()
{
    ...
    _dev_sem = _dev->get_semaphore();

    _dev->register_periodic_callback(5000, FUNCTOR_BIND_MEMBER(&RCOutput_Raspilot::_update, void));
}
...
bool RCOutput_Raspilot::force_safety_on(void) {
    if (!_dev_sem->take(0)) {
        return false;
    }
...
void RCOutput_Raspilot::force_safety_off(void) {
    if (!_dev_sem->take(0)) {
        return;
    }
    struct IOPacket _dma_packet_tx, _dma_packet_rx;
